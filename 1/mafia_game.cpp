...
private:
    ...
    void handleDoctorNightAction(int userId) {
        ...
        while (!validateTargetDoctor(target, userId) || target == doctor->getPreviousTarget()) {
    ...
    bool validateTargetDoctor(int targetId, int userId) {
    ...
    void nightPhase(int round) {
        ...
        std::vector<SmartPointer<Player>> alpl;
        for (const auto& [id, player] : alivePlayers) {
            alpl.push_back(player);
        }   

        if(donId != -1){
        Mafia* donMafiaPlayer = dynamic_cast<Mafia*>(alivePlayers[donId].get());
        victimMafia = donMafiaPlayer->getKillTarget();
        }
        // Поиск маньяка и комиссара
        auto maniacPl = alpl | std::views::filter([](const auto& entry) {
            return entry->getRole() == Role::MANIAC;
        });

        auto commissionerPl = alpl | std::views::filter([](const auto& entry) {
            return entry->getRole() == Role::COMMISSIONER;
        });
        ...
        if (!std::ranges::empty(maniacPl)) {
            auto maniacPlayer = maniacPl.front();
            victimManiac = maniacPlayer->getKillTarget();
        }

        if (!std::ranges::empty(commissionerPl)) {
            auto commissionerPlayer = commissionerPl.front();
    ...
    void processKill(int victim, const std::string& killer, const std::pair<int, int> protectedPlayer) {
        if (victim != -1 && victim != protectedPlayer.first && victim != protectedPlayer.second && alivePlayers.find(victim) != alivePlayers.end()) {
            ...
            if (alivePlayers[victim]->getRole() == Role::MAFIA) {
                auto pl = dynamic_cast<Mafia*>(alivePlayers[victim].get());
                if (pl->getRoleName() == "Дон мафии") {
                    for (const auto& [id, player] : alivePlayers) {
                        if (player->getRole() == Role::MAFIA && id != victim) {
                            auto pl = dynamic_cast<Mafia*>(alivePlayers[victim].get());
                            pl->is_don = true;
                            alivePlayers[id] = SmartPointer<Player>(new Mafia(id, true));
                            break;
                        }
                    }
                }
